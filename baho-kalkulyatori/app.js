// app.js — Baho kalkulyatori. Andoza sifatida yozilgan: keyingi har qanday
// tool shu faylning tuzilishini nusxa oladi (mountToolShell -> shared/data.js -> UI).
import { mountToolShell } from "../shared/shell.js";
import { listStudents, getToolDoc, setToolDoc, addToToolArray } from "../shared/data.js";
...
// ---------- Umumiy students ro'yxatidan o'qish (data.js orqali, boshqa hech qayerdan) ----------
async function loadStudents() {
  syncStatus.textContent = "Yuklanmoqda...";
  try {
    students = await listStudents(user.uid);
    if (students.length === 0) {
      studentSelect.innerHTML = `<option value="">Avval "Ism tanlash" tool'ida ro'yxat kiriting</option>`;
